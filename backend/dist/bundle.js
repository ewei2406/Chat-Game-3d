...
const Matrix_1 = __importDefault(__webpack_require__(/*! ../Matrix/Matrix */ "./src/Matrix/Matrix.ts"));
const PositionPoint_1 = __importDefault(__webpack_require__(/*! ../Point/PositionPoint */ "./src/Point/PositionPoint.ts"));
const RotationPoint_1 = __importDefault(__webpack_require__(/*! ../Point/RotationPoint */ "./src/Point/RotationPoint.ts"));
class Camera {
    constructor(canvas, x, y, z) {
        this.near = 0.1;
        this.far = 300;
        this.f = 256;
        this.rotationOffset = new RotationPoint_1.default(0, 0, 0);
        this.position = new PositionPoint_1.default(x, y, z);
        this.rotation = new RotationPoint_1.default(0, 0, 0);
        this.canvas = canvas;
    ...
    updateM() {
        ...
        const R_yaw = new Matrix_1.default([
            [y1, y2, 0],
            [-y2, y1, 0],
            [0, 0, 1]
        ]);
        ...
        const R_pitch = new Matrix_1.default([
            [p1, 0, -p2],
            [0, 1, 0],
            [p2, 0, p1]
        ]);
        ...
        const R_roll = new Matrix_1.default([
            [1, 0, 0],
            [0, r1, -r2],
            [0, r2, r1],
        ]);
        const R = R_roll.multiply(R_yaw).multiply(R_pitch);
        const I_C = new Matrix_1.default([
...
class Canvas {
    ...
    point(x, y, size = 2, color = "black") {
        this.ctx.fillStyle = color;
        this.ctx.fillRect(x - size / 2, y - size / 2, size, size);
    }
    line(x0, y0, x1, y1, color = "green", width = 1) {
    ...
    text(x0, y0, text, size = 10, color = "black") {
        this.ctx.fillStyle = color;
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
class Edge {
    constructor(start, stop, color = "black", thickness = 0.2) {
        this.start = start;
        this.stop = stop;
        this.color = color;
    ...
    draw(camera) {
        ...
        const avgScale = Math.min(startScale, stopScale);
        if (startEmbedding && stopEmbedding) {
            camera.canvas.line(startEmbedding.getValue(0, 0), startEmbedding.getValue(1, 0), stopEmbedding.getValue(0, 0), stopEmbedding.getValue(1, 0), this.color, this.thickness * avgScale);
        }
    }
    transform(t) {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const Edge_1 = __importDefault(__webpack_require__(/*! ../Edge/Edge */ "./src/Edge/Edge.ts"));
const BoundingPoint_1 = __importDefault(__webpack_require__(/*! ../Point/BoundingPoint */ "./src/Point/BoundingPoint.ts"));
const Point_1 = __importDefault(__webpack_require__(/*! ../Point/Point */ "./src/Point/Point.ts"));
const PositionPoint_1 = __importDefault(__webpack_require__(/*! ../Point/PositionPoint */ "./src/Point/PositionPoint.ts"));
const RotationPoint_1 = __importDefault(__webpack_require__(/*! ../Point/RotationPoint */ "./src/Point/RotationPoint.ts"));
class Entity {
    constructor(x, y, z, yaw = 0, pitch = 0, roll = 0, id = "-1") {
        this.axis_X = new Edge_1.default(new Point_1.default(0, 0, 0), new Point_1.default(5, 0, 0), "red");
        this.axis_Y = new Edge_1.default(new Point_1.default(0, 0, 0), new Point_1.default(0, 5, 0), "green");
        this.axis_Z = new Edge_1.default(new Point_1.default(0, 0, 0), new Point_1.default(0, 0, 5), "blue");
        this.pointer = new Edge_1.default(new Point_1.default(0, 0, 0), new Point_1.default(5, 0, 0), "purple");
        this.position = new PositionPoint_1.default(x, y, z);
        this.rotation = new RotationPoint_1.default(yaw, pitch, roll);
        this.hitbox = new BoundingPoint_1.default(10, 20, 10);
        this.id = id;
    ...
    drawName(camera) {
        ...
        if (embedding) {
            camera.canvas.text(embedding.getValue(0, 0), embedding.getValue(1, 0), this.nickname, 2 * scale);
        }
    }
    draw(camera) {
    }
...
const PositionPoint_1 = __importDefault(__webpack_require__(/*! ../Point/PositionPoint */ "./src/Point/PositionPoint.ts"));
const RotationPoint_1 = __importDefault(__webpack_require__(/*! ../Point/RotationPoint */ "./src/Point/RotationPoint.ts"));
const Entity_1 = __importDefault(__webpack_require__(/*! ./Entity */ "./src/Entity/Entity.ts"));
class Player extends Entity_1.default {
    constructor(x, y, z, yaw, pitch, roll, id = "-1") {
        ...
        this.dispEdges = [
            new Edge_1.default(new Point_1.default(-2, 0, 0), new Point_1.default(2, 0, 0), color, 1),
            new Edge_1.default(new Point_1.default(0, -2, 0), new Point_1.default(0, 2, 0), color, 1),
            new Edge_1.default(new Point_1.default(0, 0, -2), new Point_1.default(0, 0, 2), color, 1)
        ];
        this.V = new PositionPoint_1.default(0, 0, 0, 0);
        this.A = new PositionPoint_1.default(0, -0.06, 0, 0);
        this.V_R = new RotationPoint_1.default(0, 0, 0, 0);
    ...
    updateInput(input) {
        ...
        if (input.FORWARD)
            ...
            this.V.setX(0);
        if (input.UP && this.position.getY() === 10)
            this.V.setY(1.2);
        // else this.moveUp(0)
...
class TextEntity extends Entity_1.default {
    constructor(x, y, z, message, lifespan = 500) {
    ...
    draw(camera) {
        ...
        if (embedding) {
            camera.canvas.text(embedding.getValue(0, 0), embedding.getValue(1, 0), this.message, 2 * scale, `rgba(0, 0, 0, ${1 - (Math.max(0, (100 - this.lifespan) / 100))})`);
        }
    }
}
...
class Matrix {
    constructor(contents) {
        ...
        for (let r = 0; r < contents.length; r++) {
            if (contents[r].length != this.num_cols)
                throw new Error('non-rectangular matrix creation error');
        }
    ...
    multiplyRow(r, factor) {
        this.contents[r] = this.contents[r].map(i => i * factor);
    }
    multiplyCol(c, factor) {
        for (let r = 0; r < this.num_rows; r++) {
            this.contents[r][c] *= factor;
        }
    }
    addCol(values) {
        if (values.length != this.num_rows)
            throw new Error('Adding new column of wrong size error');
        for (let r = 0; r < this.num_rows; r++) {
    ...
    dotProduct(other) {
        if (this.num_cols != other.num_cols || this.num_rows != other.num_rows)
            throw new Error('incompatible matrix size dot product error');
        let sum = 0;
        for (let r = 0; r < this.num_rows; r++) {
            for (let c = 0; c < this.num_cols; c++) {
    ...
    multiply(other) {
        if (this.num_cols != other.num_rows)
            throw new Error('incompatible matrix size multiplication error');
        const acc = [];
    ...
    clone() {
        const newContents = Array(this.num_rows).fill(0).map(() => Array(this.num_cols).fill(0));
        for (let r = 0; r < this.num_rows; r++) {
            for (let c = 0; c < this.num_cols; c++) {
...
const PositionPoint_1 = __importDefault(__webpack_require__(/*! ../Point/PositionPoint */ "./src/Point/PositionPoint.ts"));
class Cube {
    constructor(x, y, z, dx = 10, dy = 10, dz = 10, color = "black", thickness = 0.2) {
        this.verticies = [];
        this.edges = [];
        this.position = new PositionPoint_1.default(x, y, z);
        for (let p_x = x; p_x <= x + dx; p_x += dx) {
            for (let p_y = y; p_y <= y + dy; p_y += dy) {
                for (let p_z = z; p_z <= z + dz; p_z += dz) {
                    this.verticies.push(new PositionPoint_1.default(p_x, p_y, p_z));
                }
            }
        }
        this.edges.push(new Edge_1.default(new Point_1.default(x, y, z), new Point_1.default(x + dx, y, z), color, thickness));
        this.edges.push(new Edge_1.default(new Point_1.default(x, y, z), new Point_1.default(x, y, z + dz), color, thickness));
        this.edges.push(new Edge_1.default(new Point_1.default(x, y, z), new Point_1.default(x, y + dy, z), color, thickness));
        this.edges.push(new Edge_1.default(new Point_1.default(x + dx, y + dy, z), new Point_1.default(x, y + dy, z), color, thickness));
        this.edges.push(new Edge_1.default(new Point_1.default(x + dx, y + dy, z), new Point_1.default(x + dx, y + dy, z + dz), color, thickness));
        this.edges.push(new Edge_1.default(new Point_1.default(x + dx, y + dy, z), new Point_1.default(x + dx, y, z), color, thickness));
        this.edges.push(new Edge_1.default(new Point_1.default(x + dx, y, z + dz), new Point_1.default(x + dx, y, z), color, thickness));
        this.edges.push(new Edge_1.default(new Point_1.default(x + dx, y, z + dz), new Point_1.default(x, y, z + dz), color, thickness));
        this.edges.push(new Edge_1.default(new Point_1.default(x + dx, y, z + dz), new Point_1.default(x + dx, y + dy, z + dz), color, thickness));
        this.edges.push(new Edge_1.default(new Point_1.default(x, y + dy, z + dz), new Point_1.default(x, y + dy, z), color, thickness));
        this.edges.push(new Edge_1.default(new Point_1.default(x, y + dy, z + dz), new Point_1.default(x, y, z + dz), color, thickness));
        this.edges.push(new Edge_1.default(new Point_1.default(x, y + dy, z + dz), new Point_1.default(x + dx, y + dy, z + dz), color, thickness));
    }
    draw(camera) {
...
const Edge_1 = __importDefault(__webpack_require__(/*! ../Edge/Edge */ "./src/Edge/Edge.ts"));
const Point_1 = __importDefault(__webpack_require__(/*! ../Point/Point */ "./src/Point/Point.ts"));
const MakeFlower = (x, y, z, color) => {
    const objs = [];
    const bud = new Point_1.default(x, y + 2, z, 1, color, 1);
    objs.push(new Edge_1.default(bud, new Point_1.default(x, y, z, 1), 'green'));
    objs.push(bud);
    return objs;
};
exports["default"] = MakeFlower;


/***/ }),

/***/ "./src/Object/MakeHouse.ts":
/*!*********************************!*\
  !*** ./src/Object/MakeHouse.ts ***!
  \*********************************/
/***/ (function(__unused_webpack_module, exports, __webpack_require__) {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const Cube_1 = __importDefault(__webpack_require__(/*! ./Cube */ "./src/Object/Cube.ts"));
const MakeHouse = (x, y, z, size, width, depth, height) => {
    const cubes = [];
    // Frame
    cubes.push(new Cube_1.default(x, y, z, size * width, size * height, size * depth, 'black', size / 10));
    // cubes.push(new Cube(x + size, y, z + size, size * (width - 2), size * (height - 1), size * (width - 2)))
    // Door
    cubes.push(new Cube_1.default(x + size, y, z, size, size * 2, size * 0.1, 'brown', size / 15));
    cubes.push(new Cube_1.default(x + (size * 1.7), y + size, z - size * 0.1, size * 0.2, size * 0.2, size * 0.2, 'gold', size / 15));
    cubes.push(new Cube_1.default(x + size * (width - 2), y + size * height, z + size * (depth - 2), size, size * 2, size, 'grey', size / 10));
    return cubes;
};
exports["default"] = MakeHouse;


/***/ }),

/***/ "./src/Object/MakeTree.ts":
/*!********************************!*\
  !*** ./src/Object/MakeTree.ts ***!
  \********************************/
/***/ (function(__unused_webpack_module, exports, __webpack_require__) {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const Cube_1 = __importDefault(__webpack_require__(/*! ./Cube */ "./src/Object/Cube.ts"));
const MakeTree = (x, y, z, size = 10, height = 5, topSize = 5) => {
    const cubes = [];
    cubes.push(new Cube_1.default(x, y, z, size, size * height, size, 'brown', size / 10));
    y += height * (size);
    x += size / 2;
    z += size / 2;
    for (let h = topSize; h > 0; h--) {
        cubes.push(new Cube_1.default(x - (size * h / 2), y, z - (size * h / 2), size * h, size / 2, size * h, 'green', size / 10));
        y += size / 2;
    }
    return cubes;
};
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const Matrix_1 = __importDefault(__webpack_require__(/*! ../Matrix/Matrix */ "./src/Matrix/Matrix.ts"));
const Point_1 = __importDefault(__webpack_require__(/*! ./Point */ "./src/Point/Point.ts"));
class BoundingPoint extends Point_1.default {
    constructor(width, height, depth, norm = 1) {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const ColumnVector_1 = __importDefault(__webpack_require__(/*! ../Vector/ColumnVector */ "./src/Vector/ColumnVector.ts"));
class Point extends ColumnVector_1.default {
    constructor(x, y, z, norm = 1, color = "black", size = 0.5) {
        super([x, y, z, norm]);
        this.color = color;
        this.size = size;
    }
    ...
    draw(camera) {
        ...
        if (embedding) {
            camera.canvas.point(embedding.getValue(0, 0), embedding.getValue(1, 0), this.size * scale, this.color);
        }
    }
    transform(t) {
...
const Matrix_1 = __importDefault(__webpack_require__(/*! ../Matrix/Matrix */ "./src/Matrix/Matrix.ts"));
const Point_1 = __importDefault(__webpack_require__(/*! ./Point */ "./src/Point/Point.ts"));
class PositionPoint extends Point_1.default {
    constructor(x, y, z, norm = 1) {
        super(x, y, z, norm);
    }
    getX() {
        return this.getValue(0);
...
exports["default"] = PositionPoint;


/***/ }),

/***/ "./src/Point/RotationPoint.ts":
/*!************************************!*\
  !*** ./src/Point/RotationPoint.ts ***!
  \************************************/
/***/ (function(__unused_webpack_module, exports, __webpack_require__) {
...
const Matrix_1 = __importDefault(__webpack_require__(/*! ../Matrix/Matrix */ "./src/Matrix/Matrix.ts"));
const Point_1 = __importDefault(__webpack_require__(/*! ./Point */ "./src/Point/Point.ts"));
class RotationPoint extends Point_1.default {
    constructor(yaw, pitch, roll, norm = 1) {
...
class Connection {
    constructor(onReady) {
        axios_1.default.get('/generateId').then(res => {
            this.id = res.data;
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const Matrix_1 = __importDefault(__webpack_require__(/*! ../Matrix/Matrix */ "./src/Matrix/Matrix.ts"));
class ColumnVector extends Matrix_1.default {
    constructor(contents) {
        super(contents.map(i => [i]));
    }
}
exports["default"] = ColumnVector;
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const Camera_1 = __importDefault(__webpack_require__(/*! ./Camera/Camera */ "./src/Camera/Camera.ts"));
const Point_1 = __importDefault(__webpack_require__(/*! ./Point/Point */ "./src/Point/Point.ts"));
const Canvas_1 = __importDefault(__webpack_require__(/*! ./Canvas/Canvas */ "./src/Canvas/Canvas.ts"));
const Input_1 = __importDefault(__webpack_require__(/*! ./Input/Input */ "./src/Input/Input.ts"));
const Entity_1 = __importDefault(__webpack_require__(/*! ./Entity/Entity */ "./src/Entity/Entity.ts"));
const info_1 = __importDefault(__webpack_require__(/*! ./info */ "./src/info.ts"));
const Player_1 = __importDefault(__webpack_require__(/*! ./Entity/Player */ "./src/Entity/Player.ts"));
const Connection_1 = __importDefault(__webpack_require__(/*! ./Service/Connection */ "./src/Service/Connection.ts"));
const Cube_1 = __importDefault(__webpack_require__(/*! ./Object/Cube */ "./src/Object/Cube.ts"));
const TextEntity_1 = __importDefault(__webpack_require__(/*! ./Entity/TextEntity */ "./src/Entity/TextEntity.ts"));
const MakeTree_1 = __importDefault(__webpack_require__(/*! ./Object/MakeTree */ "./src/Object/MakeTree.ts"));
const MakeHouse_1 = __importDefault(__webpack_require__(/*! ./Object/MakeHouse */ "./src/Object/MakeHouse.ts"));
const MakeFlower_1 = __importDefault(__webpack_require__(/*! ./Object/MakeFlower */ "./src/Object/MakeFlower.ts"));
const canvas = new Canvas_1.default('display');
canvas.text(40, 10, "LOADING...", 10);
(0, info_1.default)(["LOADING..."]);
const input = new Input_1.default(canvas);
const camera = new Camera_1.default(canvas, -10, -1, -10);
const origin = new Entity_1.default(0, 0, 0);
const worldObjs = [];
const K = 2 ** 17;
const lcong_randnum = (i, num = 1000) => {
...
worldObjs.push(new Cube_1.default(20, 230, 20, 30, 30, 30, 'yellow', 5));
for (let i = 100; i < 200; i += 10) {
    worldObjs.push(new Cube_1.default((lcong_randnum(i) - 0.5) * 300, 90 + (lcong_randnum(i + 1) - 0.5) * 20, (lcong_randnum(i + 1) - 0.5) * 300, 30, 10, 30, 'white', 2));
}
for (let i = 1000; i < 10000; i += 2) {
    worldObjs.push(new Point_1.default((lcong_randnum(i) - 0.5) * 200, 0, (lcong_randnum(i + 1) - 0.5) * 200, 1, 'green'));
}
for (let i = 0; i < 100; i += 2) {
    worldObjs.push(...(0, MakeFlower_1.default)((lcong_randnum(i) - 0.5) * 200, 0, (lcong_randnum(i + 1) - 0.5) * 200, 'red'));
}
for (let i = 100; i < 200; i += 2) {
    worldObjs.push(...(0, MakeFlower_1.default)((lcong_randnum(i) - 0.5) * 200, 0, (lcong_randnum(i + 1) - 0.5) * 200, 'yellow'));
}
worldObjs.push(...(0, MakeTree_1.default)(10, 0, 10, 8, 5, 5));
worldObjs.push(...(0, MakeTree_1.default)(30, 0, 50, 6, 5, 4));
worldObjs.push(...(0, MakeTree_1.default)(30, 0, 50, 6, 5, 3));
worldObjs.push(...(0, MakeTree_1.default)(-30, 0, -50, 3, 10, 4));
// worldObjs.push(...MakeTree(30, 0, -50, 3, 30, 5))
worldObjs.push(...(0, MakeTree_1.default)(-10, 0, 10, 1, 4, 4));
worldObjs.push(...(0, MakeHouse_1.default)(60, 0, 10, 7.5, 5, 5, 4));
worldObjs.push(...(0, MakeHouse_1.default)(-80, 0, 50, 5, 5, 5, 10));
worldObjs.push(...(0, MakeHouse_1.default)(40, 0, -50, 0.5, 5, 5, 5));
worldObjs.push(...(0, MakeTree_1.default)(45, 0, -50, 0.5, 5, 5));
const connection = new Connection_1.default(() => {
    console.log("Connected!");
    const player = new Player_1.default(0, 0, 0, 0, 0, 0, connection.id);
    let prev = player.toPlayerData();
    connection.sendPlayerData(player);
    // nickname
    document.getElementById('nickname').value = player.nickname;
    document.getElementById('nickname').onkeyup = (e) => {
        player.nickname = e.target.value;
    };
    // FOV
    document.getElementById('FOVSlider').value = camera.f.toString();
    document.getElementById('FOVSlider').oninput = (e) => {
        camera.f = Number(e.target.value);
    };
    // RenderSlider
    ...
    document.getElementById('reset').onclick = (e) => {
        camera.f = 256;
        camera.far = 300;
        document.getElementById('renderSlider').value = "300";
        document.getElementById('FOVSlider').value = "256";
    };
    // Chat
    document.getElementById('sendMessage').onkeyup = (e) => {
        if (e.key === "Enter") {
            const msg = e.target.value;
            connection.sendMessage(msg, player);
            messageEntities.push(new TextEntity_1.default(player.position.getX(), player.position.getY(), player.position.getZ(), msg));
    ...
    connection.onRecievePlayerData(pd => {
        ...
        pd.map(playerData => {
            if (playerData.id in otherPlayers) {
                ...
                otherPlayers[playerData.id] = new Player_1.default(playerData.pos[0], playerData.pos[1], playerData.pos[2], playerData.rot[0], playerData.rot[1], playerData.rot[2], playerData.id);
            }
            if (playerData.nickname)
                otherPlayers[playerData.id].nickname = playerData.nickname;
        });
    });
    let messages = [];
    let messageEntities = [];
    connection.onRecieveMessage(msg => {
        ...
        if (sentPlayer) {
            messageEntities.push(new TextEntity_1.default(sentPlayer.position.getX(), sentPlayer.position.getY(), sentPlayer.position.getZ(), msg.contents));
        }
        messages.push(msg);
        document.getElementById('chatBox').innerHTML = messages.map(m => `[${m.timestamp.toString()}] ${m.nickname} (id:${m.playerId}): ${m.contents}`).join("<br/>");
    });
    let t0 = Date.now();
    let count = 0;
    const main = () => {
        ...
        // Communicate
        let dt = Date.now() - t0;
        (0, info_1.default)(['FPS: ' + Math.round(1000 / dt).toString().slice(0, 3), 'Players connected: ' + (Object.keys(otherPlayers).length + 1).toString()]);
        t0 = Date.now();
        count += dt;
        if (count >= 1000 / 30) {
            if (!player.isEqualToData(prev))
                connection.sendPlayerData(player);
